...
export const componentCategories: ComponentCategory[] = [
  {
    key: 'interactive',
    title: '交互',
    hint: '点击 / 悬停 / 键盘驱出，就地改变界面状态',
    // 与「浮层」一起放右列：左列偏「内容型」（展示 + 输入），右列偏「会动 / 浮起」
    side: 'right',
    names: [
      'accordion',
      'button',
  ...
  },
  {
    key: 'overlays',
    title: '浮层',
    hint: '触发后浮在页面之上：弹窗 / 抽屉 / 气泡 / 菜单',
    // 与「交互」一起放右列（本类与「交互」的区别：内容默认不存在，是触发后才浮出来的）
    side: 'right',
    names: [
      'alert-dialog',
      'context-menu',
  ...
  },
  {
    ...
    // 左列最外侧（2xl 起与「展示」并排，输入在展示左边）；数组顺序 = 视觉顺序
    names: [
      'calendar',
      'checkbox',
      'combobox',
