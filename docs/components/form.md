...
const demo0 = `    <FormField v-slot="{ componentField }">
      <FormItem>
...
const demo1 = `    <FormField v-slot="{ field }">
      <FormItem>
        <FormLabel>Username</FormLabel>
        <FormControl>
...
The Form component is an abstraction over the `vee-validate` library. Going forward, we recommend using the [`<Field />`](/components/field.html) component to build forms. See the [Form](/components/forms.html) documentation for more information.
