...
<script setup>
import { Marker, MarkerContent, MarkerIcon, } from '../../src/components/ui/marker'

const demo0 = `import { Marker, MarkerContent, MarkerIcon, } from '@/components/ui/marker'
...
- Pairs with the [`shimmer`](/components/utilities/shimmer.html) utility for streaming status text
...
Set `role="status"` and include a [`Spinner`](/components/spinner.html) for streaming or in-progress markers so updates are announced.
...
Add the [`shimmer`](/components/utils/shimmer.html) utility class to `MarkerContent` for an animated streaming-text effect. The utility ships with the `shadcn` package — see the shimmer docs for installation.
