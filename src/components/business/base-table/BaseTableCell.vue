...
const optionLabel = () => {
  ...
  if (!opts) return props.row[props.col.field]
  return opts.find(opt => opt.value === props.row[props.col.field])?.label ?? props.row[props.col.field]
}
</script>
...
<template>
  <template v-if="col.type === 'markdown'">
    <div class="overflow-hidden break-words" :style="{ maxHeight: `${(maxLines ?? 3) * 1.5}em` }">
      <MdPreview :model-value="String(row[col.field] ?? '')" />
    </div>
  </template>
  <template v-else-if="col.type === 'image'">
    <img :src="imgUrl(String(row[col.field] ?? ''))" alt="" class="max-h-24 max-w-28 object-contain" />
  </template>
